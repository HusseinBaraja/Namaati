...
const handleDrawerWidth = () => {
  ...
  drawerProps.icon =
    drawerProps.railWidth === 256 ? icons.arrowLeft : icons.arrowRight;
...
// Function to update subheader transition properties
const updateSubheaderTransition = (duration, delay) => {
  subheaderProps.transitionDuration = duration;
  subheaderProps.transitionDelay = delay;
};

// Function to update menu item transition properties
const updateMenuItemTransition = (duration, delay) => {
  menuItemProps.transitionDuration = duration;
  menuItemProps.transitionDelay = delay;
};
...
<template>
  ...
  >
          ...
          >
            ...
            >
              <div
                ...
                :class="{
                  'absolute left-3':
                    drawerProps.railWidth === 256 || drawerProps.isSettling,
                  'relative left-1/2 transform -translate-x-1/2':
                    drawerProps.railWidth === 64 && !drawerProps.isSettling,
                }"
              >
                <svg
    ...
    <button
      ...
      class="absolute top-1/2 transform -translate-y-1/2 bg-white rounded-full w-6 h-6 shadow-md hover:bg-gray-100 transition-colors duration-300 flex items-center justify-center"
      :style="{ left: `${drawerProps.railWidth - 3}px` }"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
